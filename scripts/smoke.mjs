...
(async () => {
  ...
  if (RUN_LOCAL) {
    ...
    try { const r = await fetch(BASE + '/api/health'); if (r.ok) preExisting = true; } catch {}
    if (!preExisting) {
      ...
      while (!ready && Date.now() - startedAt < 30000) {
        try { const r = await fetch(BASE + '/api/health'); if (r.ok) ready = true; else await delay(500); }
        catch { await delay(500); }
      }
      if (!ready) { console.error('Server failed to become ready within 30s'); process.exit(1); }
    } else {
  ...
  await step('dashboard authed', async () => {
    const res = await fetch(BASE + '/dashboard', { headers: { cookie: cookieJar } });
  ...
  await step('admin login', async () => {
    ...
    if (res.status === 401) {
      console.warn('⚠ admin login unauthorized (check ADMIN_SECRET). Continuing.');
      return;
    }
  ...
  await step('card request route', async () => {
    const { res } = await jsonFetch('/api/cards/request', { method: 'POST', body: JSON.stringify({}) });
  ...
  for (const r of results) {
    console.log(`${r.ok ? '✔' : '✖'} ${r.name} (${r.ms}ms)` + (r.error ? ` - ${r.error}` : ''));
  }
