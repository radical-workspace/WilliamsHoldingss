...
export const withdrawalSchema = z
	...
	.superRefine((val, ctx) => {
		...
		if (isSwift) {
			...
			if (
				!/^SWIFT:[A-Z0-9]{8,11}:[A-Z0-9]{11,34}:.+/.test(
					val.destination_address,
				)
			) {
				ctx.addIssue({
					code: z.ZodIssueCode.custom,
					message: "Invalid SWIFT/IBAN details",
					path: ["destination_address"],
				});
			}
			return;
		...
		if (
			!/^(0x[a-fA-F0-9]{40}|bc1[a-zA-Z0-9]{25,48}|[13][a-km-zA-HJ-NP-Z1-9]{25,34})$/.test(
				val.destination_address,
			)
		) {
			ctx.addIssue({
