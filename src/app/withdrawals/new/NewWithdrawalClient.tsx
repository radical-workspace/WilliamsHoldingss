...
function basicAddrOk(addr: string) {
	return /^(0x[a-fA-F0-9]{40}|bc1[a-zA-Z0-9]{25,48}|[13][a-km-zA-HJ-NP-Z1-9]{25,34})$/.test(
		addr,
	);
}
...
export default function NewWithdrawalClient() {
	...
	const assets = useMemo(
		() => Array.from(new Set(items.map((i) => i.asset))),
		[items],
	);
	const [asset, setAsset] = useState("");
	const networks = useMemo(
		() => items.filter((i) => i.asset === asset).map((i) => i.network),
		[items, asset],
	);
	...
	useEffect(() => {
		(async () => {
			...
			if (arr.length) {
				...
				const qpNetwork =
					params.get("network") ||
					arr.find((i: any) => i.asset === qpAsset)?.network ||
					arr[0].network;
				setAsset(qpAsset);
	...
	const rules: Record<
		string,
		{ min?: number; regex?: Record<string, RegExp> }
	> = {
		BTC: {
			min: 0.00001,
			regex: { bitcoin: /^(bc1|[13])[a-zA-HJ-NP-Z0-9]{25,}$/ },
		},
	...
	const isAchValid =
		achName.trim().length > 1 &&
		/^\d{9}$/.test(achRouting) &&
		/^\d{5,}$/.test(achAccount);
	const isAddrValid = netRegex ? netRegex.test(address) : basicAddrOk(address);
	const formValid =
		method === "BANK" ? isAchValid : Boolean(asset && network) && isAddrValid;
	const amountValid =
		amountNum >= Math.max(0.00000001, min) && amountNum <= (available || 0);
			...
			<Box
				...
				onSubmit={async (e) => {
					...
					const payload =
						method === "CRYPTO"
							...
							: {
									...
									network: "ACH",
									amount: amountNum,
									destination_address: `ACH:${achRouting}:${achAccount}:${achName}`,
							  };
					...
					if (!res.ok) setMessage(data.error || "Submission failed");
						...
						setMessage(
							"Submitted! Status PENDING until admin approves and sends off-chain.",
						);
				}}
			>
				<Stack spacing={2} sx={{ maxWidth: 600 }}>
					{method === "CRYPTO" ? (
						<>
							...
							<TextField
								...
								helperText={
									Boolean(address) && !isAddrValid
										? "Enter a valid address for the selected network"
										: " "
								}
								fullWidth
								required
						...
						<>
							<TextField
								...
								helperText={
									Boolean(achName) && achName.trim().length <= 1
										? "Enter full name"
										: " "
								}
								fullWidth
							...
							<Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
								<TextField
									...
									helperText={
										Boolean(achRouting) && !/^\d{9}$/.test(achRouting)
											? "9 digits required"
											: " "
									}
									fullWidth
								...
								<TextField
									...
									value={achAccount}
									onChange={(e) =>
										setAchAccount(e.target.value.replace(/[^0-9]/g, ""))
									}
									placeholder="Checking/Savings"
									error={Boolean(achAccount) && !/^\d{5,}$/.test(achAccount)}
									helperText={
										Boolean(achAccount) && !/^\d{5,}$/.test(achAccount)
											? "At least 5 digits"
											: " "
									}
									fullWidth
					...
					>
						<TextField
							label={`Amount (Available: ${available}${
								min ? `, Min: ${min}` : ""
							})`}
							type="number"
							...
							helperText={
								Boolean(amount) && !amountValid
									? "Enter a valid amount within available balance"
									: " "
							}
							fullWidth
							required
					...
					{message && (
						<Alert
							severity={message.startsWith("Submitted") ? "success" : "error"}
						>
							{message}
						</Alert>
					)}
