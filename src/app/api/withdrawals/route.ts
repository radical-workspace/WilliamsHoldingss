...
export async function POST(req: Request) {
  ...
  const result = await prisma.$transaction(async (tx) => {
    const bal = await tx.balance.upsert({
      where: { userId_asset_network: { userId: dbUser.id, asset: asset as any, network: network as any } },
      update: {},
      create: { userId: dbUser.id, asset: asset as any, network: network as any, available: 0, locked: 0 }
    })
    if (Number(bal.available) < amount) {
      throw new Error('Insufficient available balance')
    }
    const wd = await tx.withdrawal.create({
      data: { userId: dbUser.id, asset: asset as any, network: network as any, amount: amount as any, destinationAddress: destination_address }
    })
    await tx.balance.update({
      where: { id: bal.id },
      data: { available: ((Number(bal.available) - amount) as any), locked: ((Number(bal.locked) + amount) as any) }
    })
