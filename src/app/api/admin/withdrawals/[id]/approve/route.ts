...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  const body = await req.json().catch(() => ({})) as { adminNote?: string }
    const result = await prisma.$transaction(async (tx) => {
      const wd = await tx.withdrawal.update({ where: { id }, data: { status: 'APPROVED', adminNote: body.adminNote } })
    ...
    if (Number(bal.locked) < Number(wd.amount)) throw new Error('Locked balance insufficient')
    ...
    await tx.ledger.create({
      data: {
        ...
        refType: 'Withdrawal',
        refId: wd.id,
      memo: body.adminNote ? body.adminNote : 'Admin approved withdrawal and sent funds off-chain.'
      }
    })
