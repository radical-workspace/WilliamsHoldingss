...
export default function NewDeposit() {
	...
	const addr = useMemo(
		() =>
			items.find((i) => i.asset === asset && i.network === network)?.address ||
			"",
		[items, asset, network],
	);
	...
	useEffect(() => {
		(async () => {
			try {
				...
				const j = await res
					.json()
					.catch(() => ({ items: [] as ReceivingAddress[] }));
				setItems(j.items || []);
				if ((j.items || []).length) {
	...
	const onSubmit = async (e: React.FormEvent) => {
		...
		if (method === "CRYPTO" && !addr) {
			setSeverity("error");
			setMessage(
				"No receiving address available for the selected asset/network.",
			);
			return;
		}
		...
		try {
			const body =
				method === "CRYPTO"
					? {
							...
							network,
							amount: amt,
							sent_to_address: addr,
							tx_proof: txProof || undefined,
					  }
					: {
							asset: "USD",
							network: "ACH",
							amount: amt,
							sent_to_address: `${bank.bankName} ${bank.accountNumberMasked}`,
							tx_proof: txProof || undefined,
					  };
			...
			setSeverity("success");
			setMessage(
				"Submitted! It will appear in your deposits list with status PENDING.",
			);
			setTxProof("");
			setAmount("");
		} catch (err: any) {
			...
			<Paper variant="outlined" sx={{ p: 3, mb: 3, backgroundColor: "white" }}>
				...
				{method === "CRYPTO" ? (
					<Typography color="text.secondary" sx={{ mt: 1 }}>
						Copy the address, send from your wallet, then submit this form.
					</Typography>
				) : (
					<Box sx={{ mt: 2 }}>
						...
						<Typography>
							Routing Number: <code>{bank.routingNumberMasked}</code>
						</Typography>
						<Typography>
							Account Number: <code>{bank.accountNumberMasked}</code>
						</Typography>
						{bank.notes && (
							<Typography color="text.secondary" sx={{ mt: 1 }}>
								{bank.notes}
				...
				<form onSubmit={onSubmit}>
					<Stack spacing={2}>
								...
								<FormControl fullWidth>
									...
									<Select
										...
										onChange={(e) => {
											...
											setAsset(a);
											const first = items.find((i) => i.asset === a);
											setNetwork(first ? first.network : "");
										}}
									>
										{assets.map((a) => (
						...
						<TextField
							label={
								method === "BANK"
									? "Bank reference or memo (optional)"
									: "Optional Tx Proof (URL or hash)"
							}
							placeholder={
								method === "BANK" ? "e.g., BoA reference #, memo, or note" : ""
							}
							value={txProof}
						...
						<Box>
							<Button
								type="submit"
								variant="contained"
								disabled={submitting || (method === "CRYPTO" && !addr)}
							>
